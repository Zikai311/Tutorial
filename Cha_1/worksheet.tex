...
\begin{chaptermap}\itshape Vectors are combined linearly; the dot product supplies length, angle, and orthogonality; matrices encode linear combinations and linear equations; independence and invertibility connect geometry with algebra.\end{chaptermap}
\topic{\initial{V}ECTORS AND \initial{L}INEAR \initial{C}OMBINATIONS}
\[v+w=(v_1+w_1,\ldots,v_n+w_n),\qquad cv=(cv_1,\ldots,cv_n).\]
\textbf{Linear combination and span:} For $v_1,\ldots,v_n\in\mathbb R^m$ and $c_1,\ldots,c_n\in\mathbb R$, the vector $\sum_{j=1}^n c_jv_j$ is a linear combination. Its possible values form
\[\operatorname{span}\{v_1,\ldots,v_n\}=\bigl\{\sum_{j=1}^n c_jv_j:c_j\in\mathbb R\bigr\}.\]
...
\[Ax=\sum_{j=1}^n x_ja_j=b.\]
\textbf{Linear dependence theorem:} $v_1,\ldots,v_n$ are dependent iff $\sum_{j=1}^n c_jv_j=0$ for coefficients $c_j\in\mathbb R$ that are not all zero.
\topic{\initial{D}OT \initial{P}RODUCTS AND \initial{G}EOMETRY}
\[v\cdot w=v^Tw=\sum_{i=1}^n v_iw_i,\qquad \|v\|=\sqrt{v\cdot v}.\]
...
\topic{\initial{M}ATRICES AND \initial{E}QUATIONS}
...
\[AB=(Ab_1\ \cdots\ Ab_p),\quad (AB)^T=B^TA^T,\quad A(I)=A.\]
\newpage
